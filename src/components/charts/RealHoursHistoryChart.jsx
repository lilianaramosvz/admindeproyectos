...
import styles from "../../styles/components/charts/RealHoursHistoryChart.module.css";
    ...
    <div className={styles.tooltip}>
      ...
      {payload.map((item) => (
        <div key={item.dataKey} className={styles.tooltipValue}>
          ...
          />
          {item.dataKey}: {Number(item.value ?? 0).toFixed(1)} hrs
        </div>
      ))}
    </div>
  );
...
export default function RealHoursHistoryChart() {
  const { sprints, users, loading, error } = useSprintHistoryByUser("hours");
  ...
  const totalHours = filteredSprints.reduce((acc, sprintRow) => {
    return (
      acc +
